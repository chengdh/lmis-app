 ...
 */
public class AreaSyncService extends InjectingService implements PerformSync {

    public static final String TAG = "AreaSyncService";
    @Inject
    ...
    public void performSync(Context context, Account account, Bundle extras, String authority, ContentProviderClient provider, SyncResult syncResult) {
        ...
        Log.d(TAG, "AreaSyncService->performSync()");
        Intent intent = new Intent();
        intent.setAction(SyncFinishReceiver.SYNC_FINISH);
        intent.putExtra("authority",authority);
        try {
            AreaDB db = new AreaDB(context);
            ...
            if (lmis.syncWithMethod("all", arguments)) {
                ...
                Log.d(TAG, "AreaSyncService[arguments]:" + arguments.toString());
                Log.d(TAG, "AreaSyncService->affected_rows:" + affected_rows);
