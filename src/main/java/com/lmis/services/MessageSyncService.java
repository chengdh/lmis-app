...
import com.lmis.widgets.message.MessageWidget;
...
public class MessageSyncService extends Service {
	...
	public void performSync(Context context, Account account, Bundle extras,
		...
		try {
			...
			LmisHelper oe = msgDb.getOEInstance();
			if (oe == null) {
				return;
			}
			...
			domain.add("create_date", ">=", LmisDate.getDateBefore(data_limit));

			if (!extras.containsKey("group_ids")) {
				// Last id
				JSONArray msgIds = new JSONArray();
				for (LmisDataRow row : msgDb.select()) {
          ...
          String notify_title = context.getResources().getString(com.lmis.R.string.messages_sync_notify_title);
          notify_title = String.format(notify_title,affected_rows);

          String notify_body = context.getResources().getString(com.lmis.R.string.messages_sync_notify_body);
          notify_body = String.format(notify_body,affected_rows);
					...
					mNotification.showNotification(context, notify_title,notify_body, authority,
							com.lmis.R.drawable.ic_oe_notification);
				}
	...
	private List<Integer> updateOldMessages(MessageDB db, LmisHelper oe,
			LmisUser user, List<Integer> ids) {
	...
	private void updateMessageVotes(MessageDB db, LmisHelper oe, LmisUser user,
			JSONArray ids_array) {
