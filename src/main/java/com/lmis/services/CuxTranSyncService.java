...
public class CuxTranSyncService extends InjectingService implements PerformSync {
    ...
    public void performSync(Context context, Account account, Bundle extras, String authority, ContentProviderClient provider, SyncResult syncResult) {
        Log.d(TAG, "CuxTranSyncService->performSync()");
        ...
        try {
            ...
            CuxTranHeaderDB db = new CuxTranHeaderDB(context);
            ...
            if (lmis.syncWithMethod("unread_bills", arguments)) {
                ...
                Log.d(TAG, "CuxTranSyncService[arguments]:" + arguments.toString());
                Log.d(TAG, "CuxTranSyncService->affected_rows:" + affected_rows);
                ...
                if (notification && affected_rows > 0) {
                    ...
                    String notify_title = context.getResources().getString(R.string.cux_tran_sync_notify_title);
                    notify_title = String.format(notify_title, affected_rows);

                    String notify_body = context.getResources().getString(R.string.cux_tran_sync_notify_body);
