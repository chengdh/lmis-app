...
import com.lmis.addons.message.MailGroupDB;
...
public class MailGroupSyncService extends Service {
	public static final String TAG = "MailGroupSyncService";
	...
	public void performSync(Context context, Account account, Bundle extras,
		...
		try {
			...
			db.setAccountUser(LmisAccountManager.getAccountDetail(context,
                    account.name));
			...
			if (oe != null && oe.syncWithServer(true)) {
				...
				if (followers.getOEInstance().syncWithServer(domain, true)) {
					...
					for (LmisDataRow grp : followers.select(
