...
public class InnerTransitLoadOutBarcodeParser extends BarcodeParser {
    /**
     ...
     */
    public InnerTransitLoadOutBarcodeParser(Context context, int id, int fromOrgID, int toOrgID) {
        super(context, id, fromOrgID, toOrgID, false, false, ScanHeaderOpType.INNER_TRANSIT_LOAD_OUT);
    }
    ...
    @Subscribe
    public void onInnerTransitLoadOutGetBillFromServerSuccessEvent(InnerTransitLoadOutGetBillFromServerSuccessEvent evt) throws InvalidToOrgException, DBException, BarcodeNotExistsException, BarcodeDuplicateException {
        GoodsInfo gs = evt.getmGoodsInfo();

        //判断单据类型是否正确
        String[] innerBillTypes = {"InnerTransitBill", "HandInnerTransitBill", "InnerTransitReturnBill"};
        ...
        //判断单据状态是否分拣入库状态
        if (!gs.getmState().equals("inner_transit_loaded_in")) {
