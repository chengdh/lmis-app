...
public class TagsView extends MultiTagsTextView implements
	...
	protected View getViewForObject(Object object) {
		...
		if (mCustomTagView == null) {
			...
			view = l.inflate(com.lmis.R.layout.fragment_message_receipient_tag_layout,
					tagsParentView, false);
			((TextView) view.findViewById(com.lmis.R.id.txvTagSubject)).setText(item
					.getSubject());
			if (!this.showImage) {
				view.findViewById(com.lmis.R.id.imgTagImage).setVisibility(View.GONE);
			}
			if (this.showImage && item.getImage() != null
					&& !item.getImage().equals("false")) {
				((ImageView) view.findViewById(com.lmis.R.id.imgTagImage))
