...
public class Message extends BaseFragment implements
	...
	private void init() {
		...
		mListViewAdapter = new LmisListAdapter(getActivity(),
	...
	private View handleRowView(View mView, final int position) {
		...
		txvDate.setText(LmisDate.getDate(date, TimeZone.getDefault().getID()));
		...
		if (row.getString("model").equals("mail.group")) {
			...
			if (MailGroup.mMenuGroups.containsKey("group_" + res_id)) {
				LmisDataRow grp = (LmisDataRow) MailGroup.mMenuGroups.get("group_"
						+ res_id);
	...
	public class MessagesLoader extends AsyncTask<Void, Void, Boolean> {
		...
		protected Boolean doInBackground(Void... arg0) {
			...
			mType = messageType;
			List<LmisDataRow> result = db().select(where, whereArgs, null, null,"date DESC");
			HashMap<String, LmisDataRow> parent_list_details = new HashMap<String, LmisDataRow>();
