...
public class Note extends BaseFragment implements
	...
	LmisListAdapter mNoteListAdapter = null;
	...
	private void initControls() {
		...
		mNoteListAdapter = new LmisListAdapter(getActivity(),
			...
			public View getView(int position, View convertView, ViewGroup parent) {
				...
				LmisDataRow row = (LmisDataRow) mNotesList.get(position);
				...
				LmisDataRow stage = row.getM2ORecord("stage_id").browse();
				...
				List<LmisDataRow> notetags = row.getM2MRecord("tag_ids")
						.browseEach();
				for (LmisDataRow tag : notetags) {
	...
	class NoteLoader extends AsyncTask<Void, Void, Void> {
		...
		protected Void doInBackground(Void... params) {
			...
			List<LmisDataRow> records = db().select(where, whereArgs);
	...
	public List<DrawerItem> drawerMenus(Context context) {
		...
		if (stages.isEmptyTable()) {
			...
			domain.add("user_id", "=", LmisUser.current(context).getUser_id());
			LmisHelper oe = stages.getOEInstance();
			if (oe != null) {
		...
		if (note.isInstalledOnServer()) {
			...
			if (stages.count() > 0) {
				int index = 0;
				for (LmisDataRow stage : stages.select()) {
	...
	public void onClick(View v) {
		...
		LmisEditText edtTitle = (LmisEditText) mView
	...
	public void onActivityResult(int requestCode, int resultCode, Intent data) {
		...
		case KEY_NOTE:
			if (resultCode == Activity.RESULT_OK) {
				...
				LmisDataRow row = db().select(new_id);
				mNotesList.add(row);
				mNoteListAdapter.notifiyDataChange(mNotesList);
			}
			break;
