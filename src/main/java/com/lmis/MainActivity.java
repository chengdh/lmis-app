...
public class MainActivity extends FragmentActivity implements
    ...
    if (!LmisAccountManager.hasAccounts(this) || reqForNewAccount) {
      ...
      if (!LmisAccountManager.isAnyUser(mContext)) {
        accountSelectionDialog(
            LmisAccountManager.fetchAllAccounts(mContext))
          .show();
      } else {
  ...
  private void setDrawerItems() {
    ...
    if (LmisUser.current(mContext) != null) {
      ...
      LmisDataRow partnerInfo = partner.select(LmisUser.current(mContext)
          .getPartner_id());
      if (partnerInfo != null) {
  ...
  private void setUserPicIcon(Context context) {
    if (!LmisUser.current(context).getAvatar().equals("false")) {
      Drawable profPic = new BitmapDrawable(Base64Helper.getBitmapImage(
              this, LmisUser.current(context).getAvatar()));
